...
impl Warrior {
    pub fn new(hit_points: isize, damage: usize, armor: usize) -> Warrior {
        Warrior {
            hit_points,
            damage,
            armor,
    ...
    pub fn from_raw(raw_lines: Vec<String>) -> io::Result<Warrior> {
        let regex_raw = r"[^\d]+([\d]+)";
        match &raw_lines[..] {
            [hit_points_raw, damage_raw, armor_raw] => {
                let regex =
                ...
                let captures_armor = regex.captures(armor_raw).ok_or_else(|| {
                    Error::new(
                        ErrorKind::InvalidData,
                        format!("Invalid line: {}", armor_raw),
                    )
                })?;
                ...
                let armor = captures_armor[1]
                    .parse::<usize>()
                    .map_err(|e| Error::new(ErrorKind::InvalidData, e))?;

                Ok(Warrior::new(hit_points, damage, armor))
            }
            _ => Err(Error::new(
                ErrorKind::InvalidData,
                format!(
    ...
    pub fn get_damage(&self) -> usize {
        self.damage + self.items.iter().map(|item| item.damage).sum::<usize>()
    }

    pub fn get_armor(&self) -> usize {
        self.armor + self.items.iter().map(|item| item.armor).sum::<usize>()
    }
}
